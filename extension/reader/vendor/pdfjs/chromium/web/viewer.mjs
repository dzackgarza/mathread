            ...
            escape: function escape(S) {
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    if (i === 0 && exec(FIRST_DIGIT_OR_ASCII, chr)) {
                        ...
                        if ((charCode & 63488) !== 55296) result[i] = chr; else if (charCode >= 56320 || i + 1 >= length || (charCodeAt(S, i + 1) & 64512) !== 56320) result[i] = escapeChar(chr); else {
                            result[i] = chr;
                            result[++i] = charAt(S, i);
                        }
                    }
...
function getVisibleElements({scrollEl: scrollEl, views: views, sortByVisibility: sortByVisibility = false, horizontal: horizontal = false, rtl: rtl = false}) {
...
const {AbortException: AbortException, AnnotationEditorLayer: AnnotationEditorLayer, AnnotationEditorParamsType: AnnotationEditorParamsType, AnnotationEditorType: AnnotationEditorType, AnnotationEditorUIManager: AnnotationEditorUIManager, AnnotationLayer: AnnotationLayer, AnnotationMode: AnnotationMode, AnnotationType: AnnotationType, applyOpacity: applyOpacity, build: build, ColorPicker: ColorPicker, createValidAbsoluteUrl: createValidAbsoluteUrl, CSSConstants: CSSConstants, DOMSVGFactory: DOMSVGFactory, DrawLayer: DrawLayer, FeatureTest: FeatureTest, fetchData: fetchData, findContrastColor: findContrastColor, getDocument: getDocument, getFilenameFromUrl: getFilenameFromUrl, getPdfFilenameFromUrl: getPdfFilenameFromUrl, getRGB: getRGB, getRGBA: getRGBA, getUuid: getUuid, GlobalWorkerOptions: GlobalWorkerOptions, ImageKind: ImageKind, InvalidPDFException: InvalidPDFException, isDataScheme: isDataScheme, isPdfFile: isPdfFile, isValidExplicitDest: isValidExplicitDest, makeArr: makeArr, makeMap: makeMap, makeObj: makeObj, MathClamp: MathClamp, noContextMenu: noContextMenu, normalizeUnicode: normalizeUnicode, OPS: OPS, OutputScale: OutputScale, PasswordException: PasswordException, PasswordResponses: PasswordResponses, PDFDataRangeTransport: PDFDataRangeTransport, PDFDateString: PDFDateString, PDFWorker: PDFWorker, PermissionFlag: PermissionFlag, PixelsPerInch: PixelsPerInch, RenderingCancelledException: RenderingCancelledException, renderRichText: renderRichText, ResponseException: ResponseException, setLayerDimensions: setLayerDimensions, shadow: shadow, SignatureExtractor: SignatureExtractor, stopEvent: stopEvent, SupportedImageMimeTypes: SupportedImageMimeTypes, TextLayer: TextLayer, TextLayerImages: TextLayerImages, TouchManager: TouchManager, updateUrlHash: updateUrlHash, Util: Util, VerbosityLevel: VerbosityLevel, version: version, XfaLayer: XfaLayer} = globalThis.pdfjsLib;

const INTERNAL_EVT = "f719112c-31a6-4f82-b000-a6ad49430962";
...
class PDFLinkService {
    ...
    constructor({eventBus: eventBus, externalLinkTarget: externalLinkTarget = null, externalLinkRel: externalLinkRel = null, ignoreDestinationZoom: ignoreDestinationZoom = false} = {}) {
...
async function waitOnEventOrTimeout({target: target, name: name, delay: delay = 0}) {
...
class EventBus {
    ...
    dispatch(eventName, data) {
        ...
        let extListeners;
        for (const {listener: listener, internal: internal, once: once} of eventListeners.slice(0)) {
            if (once) {
...
class BasePreferences {
    ...
    constructor() {
        this.#initializedPromise = this._readFromStorage(this.#defaults).then(({browserPrefs: browserPrefs, prefs: prefs}) => {
            AppOptions.setAll({
                ...browserPrefs,
                ...prefs
            }, true);
...
class download_manager_DownloadManager extends BaseDownloadManager {
    _triggerDownload(blobUrl, originalUrl, filename, isAttachment = false) {
...
function resolveSelectExpression(scope, {selector: selector, variants: variants, star: star}) {
...
class FluentBundle {
    constructor(locales, {functions: functions, useIsolating: useIsolating = true, transform: transform = v => v} = {}) {
        this._terms = new Map;
...
function PLATFORM() {
    const {isAndroid: isAndroid, isLinux: isLinux, isMac: isMac, isWindows: isWindows} = FeatureTest.platform;
    if (isLinux) {
        return "linux";
    }
    if (isWindows) {
...
class genericl10n_GenericL10n extends L10n {
    ...
    static async #getPaths() {
        try {
            const {href: href} = document.querySelector(`link[type="application/l10n"]`);
...
var generic_scripting_getPdfFilenameFromUrl;

async function docProperties(pdfDocument) {
    const url = "", baseUrl = "";
    const {info: info, metadata: metadata, contentDispositionFilename: contentDispositionFilename, contentLength: contentLength} = await pdfDocument.getMetadata();
    return {
        ...info,
        baseURL: baseUrl,
...
class SignatureStorage {
    ...
    async getAll() {
        if (this.#signal) {
            window.addEventListener("storage", ({key: key}) => {
                if (key === KEY_STORAGE) {
...
function setReferer(url, callback) {
    dnrRequestId ??= crypto.getRandomValues(new Uint32Array(1))[0] % 2147483648;
    if (!port) {
...
class DownloadManager extends download_manager_DownloadManager {
    _getOpenDataUrl(blobUrl, filename, dest = null) {
        let url = chrome.runtime.getURL("/content/web/viewer.html") + "?file=" + encodeURIComponent(blobUrl + "#" + filename);
        if (dest) {
            url += `#${escape(dest)}`;
        }
        return url;
...
class NewAltTextManager {
    ...
    constructor({descriptionContainer: descriptionContainer, dialog: dialog, imagePreview: imagePreview, cancelButton: cancelButton, disclaimer: disclaimer, notNowButton: notNowButton, saveButton: saveButton, textarea: textarea, learnMore: learnMore, errorCloseButton: errorCloseButton, createAutomaticallyButton: createAutomaticallyButton, downloadModel: downloadModel, downloadModelDescription: downloadModelDescription, title: title}, overlayManager, eventBus) {
        ...
        eventBus.on("enableguessalttext", ({value: value}) => {
    ...
    async #toggleGuessAltText(value, isInitial) {
        ...
        if (value) {
            const {altTextLearnMoreUrl: altTextLearnMoreUrl} = this.#uiManager.mlManager;
    ...
    #setProgress() {
        ...
        const callback = async ({detail: {finished: finished, total: total, totalLoaded: totalLoaded}}) => {
            const ONE_MEGA_BYTES = 1e6;
            totalLoaded = Math.min(.99 * total, totalLoaded);
    ...
    async editAltText(uiManager, editor, firstTime) {
        ...
        const {mlManager: mlManager} = uiManager;
        ...
        if (mlManager) {
            ({canvas: canvas, width: width, height: height, imageData: this.#imageData} = editor.copyCanvas(AI_MAX_IMAGE_DIMENSION, MAX_PREVIEW_DIMENSION, true));
            if (hasAI) {
                this.#toggleGuessAltText(await isAltTextEnabledPromise, true);
            }
        } else {
            ({canvas: canvas, width: width, height: height} = editor.copyCanvas(AI_MAX_IMAGE_DIMENSION, MAX_PREVIEW_DIMENSION, false));
        }
...
class ImageAltTextSettings {
    ...
    constructor({dialog: dialog, createModelButton: createModelButton, learnMore: learnMore, closeButton: closeButton, showAltTextDialogButton: showAltTextDialogButton}, overlayManager, eventBus, mlManager) {
        this.#dialog = dialog;
        ...
        eventBus.on("enablealttextmodeldownload", ({value: value}) => {
            if (value) {
                this.#download(false);
            } else {
                this.#delete(false);
    ...
    async open({enableGuessAltText: enableGuessAltText, enableNewAltTextWhenAddingImage: enableNewAltTextWhenAddingImage}) {
        const {enableAltTextModelDownload: enableAltTextModelDownload} = this.#mlManager;
...
class AltTextManager {
    ...
    #container;
    #telemetryData=null;
    constructor({dialog: dialog, optionDescription: optionDescription, optionDecorative: optionDecorative, textarea: textarea, cancelButton: cancelButton, saveButton: saveButton}, container, overlayManager, eventBus) {
        this.#dialog = dialog;
    ...
    #setPosition() {
        ...
        const {x: x, y: y, width: width, height: height} = this.#currentEditor.getClientDimensions();
        const MARGIN = 10;
...
class AnnotationEditorParams {
    ...
    #bindListeners({editorFreeTextFontSize: editorFreeTextFontSize, editorFreeTextColor: editorFreeTextColor, editorInkColor: editorInkColor, editorInkThickness: editorInkThickness, editorInkOpacity: editorInkOpacity, editorStampAddImage: editorStampAddImage, editorFreeHighlightThickness: editorFreeHighlightThickness, editorHighlightShowAll: editorHighlightShowAll, editorSignatureAddSignature: editorSignatureAddSignature}) {
...
class Sidebar {
    ...
    #resizeObserver;
    #prevX=0;
    constructor({sidebar: sidebar, resizer: resizer, toggleButton: toggleButton}, ltr, isResizerOnTheLeft, globalAbortSignal) {
        ...
        this.#resizeObserver = new ResizeObserver(([{borderBoxSize: [{inlineSize: inlineSize}]}]) => {
            if (!isNaN(this.#prevX)) {
    ...
    #makeSidebarResizable() {
        ...
        this.#resizer.addEventListener("pointerdown", e => {
            ...
            const {clientX: clientX} = e;
            stopEvent(e);
            this.#prevX = clientX;
            pointerMoveAC = new AbortController;
            const {signal: signal} = pointerMoveAC;
...
class CommentSidebar extends Sidebar {
    ...
    #idsToElements=null;
    #uiManager=null;
    constructor({learnMoreUrl: learnMoreUrl, sidebar: sidebar, sidebarResizer: sidebarResizer, commentsList: commentsList, commentCount: commentCount, sidebarTitle: sidebarTitle, closeButton: closeButton, commentToolbarButton: commentToolbarButton}, eventBus, linkService, popup, dateFormat, ltr, globalAbortSignal) {
        super({
            sidebar: sidebar,
    ...
    updateComment(annotation) {
        ...
        const {id: id, creationDate: creationDate, modificationDate: modificationDate, richText: richText, contentsObj: contentsObj, popupRef: popupRef} = annotation;
    ...
    addComment(annotation) {
        ...
        const {popupRef: popupRef, contentsObj: contentsObj} = annotation;
    ...
    #createCommentElement(annotation) {
        const {id: id, creationDate: creationDate, modificationDate: modificationDate, richText: richText, contentsObj: contentsObj, color: color, opacity: opacity} = annotation;
        const commentItem = document.createElement("li");
    ...
    async #commentClick({currentTarget: currentTarget}) {
        ...
        this.#popup._hide();
        const {id: id, pageIndex: pageIndex, rect: rect} = annotation;
    ...
    #commentKeydown(e) {
        const {key: key, currentTarget: currentTarget} = e;
        switch (key) {
          case "ArrowDown":
...
class CommentDialog {
    ...
    #isLTR;
    #eventBus;
    constructor({dialog: dialog, toolbar: toolbar, title: title, textInput: textInput, cancelButton: cancelButton, saveButton: saveButton}, overlayManager, eventBus, ltr) {
        this.#dialog = dialog;
    ...
    async open(uiManager, editor, posX, posY, options) {
        ...
        const {contentsObj: {str: str}, color: color, opacity: opacity} = editor.getData();
        ...
        if (editor.hasDefaultPopupPosition()) {
            const {dialogWidth: dialogWidth, dialogHeight: dialogHeight} = this._dialogDimensions;
            if (parentDimensions) {
    ...
    get _dialogDimensions() {
        ...
        style.opacity = "0";
        style.display = "block";
        const {width: width, height: height} = dialog.getBoundingClientRect();
...
class CommentPopup {
    ...
    #createPopup() {
        ...
        top.addEventListener("pointerdown", e => {
            ...
            const {target: target, clientX: clientX, clientY: clientY} = e;
    ...
    updateColor(editor) {
        ...
        const {color: color, opacity: opacity} = editor.getData();
    ...
    toggle(editor, isSelected, visibility = undefined, isEditable = true) {
        ...
        const {contentsObj: contentsObj, richText: richText, creationDate: creationDate, modificationDate: modificationDate, color: color, opacity: opacity} = editor.getData();
        ...
        container.addEventListener("focus", ({relatedTarget: relatedTarget}) => {
...
class EditorUndoBar {
    ...
    constructor({container: container, message: message, undoButton: undoButton, closeButton: closeButton}, eventBus) {
        this.#container = container;
...
class OverlayManager {
    ...
    async register(dialog, canForceClose = false) {
        ...
        dialog.addEventListener("cancel", ({target: target}) => {
...
class PDFAttachmentViewer extends BaseTreeViewer {
    ...
    _bindLink(element, {attachmentId: attachmentId, content: fallbackContent, description: description, filename: filename}) {
        if (description) {
    ...
    render({attachments: attachments, keepRenderedCapability: keepRenderedCapability = false}) {
...
class GrabToPan {
    ...
    #onMouseDown(event) {
        ...
        const boundEndPan = this.#endPan.bind(this), mouseOpts = {
            capture: true,
...
class PDFCursorTools {
    ...
    #prevActive=null;
    constructor({container: container, eventBus: eventBus, cursorToolOnLoad: cursorToolOnLoad = CursorTool.SELECT}) {
        this.container = container;
        this.eventBus = eventBus;
    ...
    #addEventListeners() {
        ...
        let annotationEditorMode = AnnotationEditorType.NONE, presentationModeState = PresentationModeState.NORMAL;
        const disableActive = () => {
        ...
        eventBus.on("annotationeditormodechanged", ({mode: mode}) => {
        ...
        eventBus.on("presentationmodechanged", ({state: state}) => {
...
class PDFDocumentProperties {
    #fieldData=null;
    constructor({dialog: dialog, fields: fields, closeButton: closeButton}, overlayManager, eventBus, l10n, fileNameLookup, titleLookup) {
        this.dialog = dialog;
        this.fields = fields;
        this.overlayManager = overlayManager;
        this.l10n = l10n;
    ...
    async open() {
        ...
        const [{info: info, metadata: metadata, contentLength: contentLength}, pdfPage] = await Promise.all([ this.pdfDocument.getMetadata(), this.pdfDocument.getPage(currentPageNumber).catch(reason => {
    ...
    async #parsePageSize(pdfPage, pagesRotation) {
        ...
        const isPortrait = isPortraitOrientation(pageSizeInches), nonMetric = NON_METRIC_LOCALES.includes(this.l10n.getLanguage());
        let sizeInches = {
...
const DIACRITICS_EXCEPTION = new Set([ 12441, 12442, 2381, 2509, 2637, 2765, 2893, 3021, 3149, 3277, 3387, 3388, 3405, 3530, 3642, 3770, 3972, 4153, 4154, 5908, 5940, 6098, 6752, 6980, 7082, 7083, 7154, 7155, 11647, 43014, 43052, 43204, 43347, 43456, 43766, 44013, 3158, 3953, 3954, 3962, 3963, 3964, 3965, 3968, 3956 ]);
...
class PDFFindController {
    ...
    #copiedPageData=null;
    #savedPageData=null;
    constructor({linkService: linkService, eventBus: eventBus, delay: delay = 250, updateMatchesCountOnProgress: updateMatchesCountOnProgress = true}) {
    ...
    scrollMatchIntoView({element: element = null, pageIndex: pageIndex = -1, matchIndex: matchIndex = -1}) {
    ...
    match(query, pageContent, pageIndex) {
        ...
        const {caseSensitive: caseSensitive, entireWord: entireWord} = this.#state;
    ...
    #onPagesEdited({pagesMapper: pagesMapper, type: type, pageNumbers: pageNumbers}) {
        if (this._extractTextPromises.length === 0) {
            return;
        }
        if (type === "copy") {
...
class PDFFindBar {
    ...
    constructor(options, mainContainer, eventBus) {
        ...
        this.bar.addEventListener("keydown", ({keyCode: keyCode, shiftKey: shiftKey, target: target}) => {
            switch (keyCode) {
              case 13:
...
class PDFHistory {
    ...
    initialize({fingerprint: fingerprint, resetHistory: resetHistory = false, updateUrl: updateUrl = false}) {
        ...
        if (!this.#isValidState(state, true) || resetHistory) {
            const {hash: hash, page: page, rotation: rotation} = this.#parseCurrentHash(true);
    ...
    push({namedDest: namedDest = null, explicitDest: explicitDest, pageNumber: pageNumber}) {
        if (!this._initialized) {
            return;
        }
    ...
    #pushOrReplaceState(destination, forceReplace = false) {
        ...
        if (this._updateUrl && destination?.hash) {
            const {href: href, protocol: protocol} = document.location;
            if (protocol !== "file:") {
    ...
    #popState({state: state}) {
        ...
        if (state?.chromecomState && !this.#isValidState(state) || !state) {
            this._uid++;
            const {hash: hash, page: page, rotation: rotation} = this.#parseCurrentHash();
            this.#pushOrReplaceState({
                hash: hash,
                page: page,
                rotation: rotation
    ...
    #bindEvents() {
        ...
        const {signal: signal} = this.#eventAC;
...
class PDFLayerViewer extends BaseTreeViewer {
    ...
    render({optionalContentConfig: optionalContentConfig, pdfDocument: pdfDocument}) {
        if (this._optionalContentConfig) {
            this.reset();
        }
...
class PDFOutlineViewer extends BaseTreeViewer {
    ...
    _bindLink(element, {url: url, newWindow: newWindow, action: action, attachmentId: attachmentId, attachment: attachment, dest: dest, setOCGState: setOCGState}) {
...
class PDFPresentationMode {
    ...
    #windowAC=null;
    constructor({container: container, pdfViewer: pdfViewer, eventBus: eventBus}) {
        this.container = container;
        this.pdfViewer = pdfViewer;
        this.eventBus = eventBus;
    ...
    #addWindowListeners() {
        ...
        const {signal: signal} = this.#windowAC;
...
class XfaLayerBuilder {
    #cancelled=false;
    div=null;
    constructor({pdfPage: pdfPage, annotationStorage: annotationStorage = null, linkService: linkService, xfaHtml: xfaHtml = null}) {
        this.pdfPage = pdfPage;
        this.annotationStorage = annotationStorage;
        this.linkService = linkService;
        this.xfaHtml = xfaHtml;
    }
    async render({viewport: viewport, intent: intent = "display"}) {
        let xfaHtml;
        if (intent === "print") {
...
class PDFPrintService {
    constructor({pdfDocument: pdfDocument, pagesOverview: pagesOverview, printContainer: printContainer, printResolution: printResolution, printAnnotationStoragePromise: printAnnotationStoragePromise = null}) {
        this.pdfDocument = pdfDocument;
...
class PDFRenderingQueue {
    ...
    getHighestPriority(visible, views, scrolledDown, preRenderExtra = false, ignoreDetailViews = false) {
        ...
        if (!ignoreDetailViews) {
            for (let i = 0; i < numVisible; i++) {
                const {detailView: detailView} = visibleViews[i].view;
...
class PDFScriptingManager {
    ...
    constructor({eventBus: eventBus, externalServices: externalServices = null, docProperties: docProperties = null}) {
        this.#eventBus = eventBus;
        this.#externalServices = externalServices;
    ...
    async setDocument(pdfDocument) {
        ...
        eventBus.on("pagechanging", ({pageNumber: pageNumber, previous: previous}) => {
        ...
        eventBus.on("pagerendered", ({pageNumber: pageNumber}) => {
    ...
    async #updateFromSandbox(detail) {
        ...
        const {id: id, siblings: siblings, command: command, value: value} = detail;
        if (!id) {
            switch (command) {
              case "clear":
                console.clear();
...
var pdf_text_extractor_internalOpt;
...
class PdfTextExtractor {
    ...
    constructor(externalServices, pdfViewer, eventBus) {
        ...
        window.addEventListener("requestTextContent", ({detail: detail}) => {
...
class Menu {
    ...
    #openMenu() {
        ...
        window.addEventListener("pointerdown", ({target: target}) => {
    ...
    #onFocusOut({relatedTarget: relatedTarget}) {
    ...
    #setUpMenu() {
        ...
        const {signal: signal} = this.#menuAC;
...
class PDFThumbnailView extends RenderableView {
    ...
    constructor({container: container, eventBus: eventBus, id: id, defaultViewport: defaultViewport, optionalContentConfigPromise: optionalContentConfigPromise, linkService: linkService, renderingQueue: renderingQueue, maxCanvasPixels: maxCanvasPixels, maxCanvasDim: maxCanvasDim, pageColors: pageColors, enableSplitMerge: enableSplitMerge = false}) {
        super();
        this.id = id;
    ...
    clone(container, id) {
        ...
        const {imageContainer: imageContainer} = this;
    ...
    reset() {
        ...
        this.#updateDims();
        const {image: image, imageContainer: imageContainer} = this;
        const url = image.src;
        if (url) {
            URL.revokeObjectURL(url);
            image.src = "";
    ...
    async #convertCanvasToImage(canvas) {
        ...
        const {imageContainer: imageContainer, image: image} = this;
        const {promise: promise, resolve: resolve} = Promise.withResolvers();
    ...
    async draw() {
        ...
        const {pageColors: pageColors, pdfPage: pdfPage} = this;
        if (!pdfPage) {
            this.renderingState = RenderingStates.FINISHED;
            throw new Error("pdfPage is not loaded");
        }
        this.renderingState = RenderingStates.RUNNING;
        const {canvas: canvas, transform: transform} = this.#getPageDrawContext(DRAW_UPSCALE_FACTOR);
    ...
    setImage(pageView) {
        ...
        const {thumbnailCanvas: canvas, pdfPage: pdfPage, scale: scale} = pageView;
        if (!canvas) {
            return;
        }
        if (!this.pdfPage) {
    ...
    #reduceImage(img) {
        const {canvas: canvas} = this.#getPageDrawContext(1);
        const ctx = canvas.getContext("2d", {
            alpha: false,
...
class PDFThumbnailViewer {
    ...
    #newBadge=null;
    constructor({container: container, eventBus: eventBus, linkService: linkService, renderingQueue: renderingQueue, maxCanvasPixels: maxCanvasPixels, maxCanvasDim: maxCanvasDim, pageColors: pageColors, abortSignal: abortSignal, enableMerge: enableMerge, enableSplitMerge: enableSplitMerge, enableNewBadge: enableNewBadge, statusBar: statusBar, undoBar: undoBar, waitingBar: waitingBar, manageMenu: manageMenu, addFileComponent: addFileComponent}) {
        ...
        if (this.#enableSplitMerge && manageMenu) {
            const {button: menuButton, menu: menu, copy: copy, cut: cut, delete: del, exportSelected: exportSelected} = manageMenu;
            if (enableNewBadge) {
            ...
            eventBus.on("editingaction", ({name: name}) => {
                switch (name) {
                  case "copyPage":
                    this.#copyPages();
                    break;
            ...
            if (this.#enableMerge && addFileComponent) {
                const {picker: picker, button: button} = addFileComponent;
    ...
    static #fitImageDimensions(width, height, {minSide: minSide = 0, maxSide: maxSide}) {
    ...
    static async #fileToImageBitmap(file) {
        ...
        if (file.type !== "image/svg+xml") {
            ...
            const {width: width, height: height} = PDFThumbnailViewer.#fitImageDimensions(bitmap.width, bitmap.height, {
        ...
        try {
            ...
            image.src = url;
            await image.decode();
            const {width: width, height: height} = PDFThumbnailViewer.#fitImageDimensions(image.naturalWidth || MIN_RASTER_SIDE, image.naturalHeight || MIN_RASTER_SIDE, {
    ...
    #onStartDragging(draggedThumbnail) {
        ...
        for (const selected of this.#selectedPages) {
            ...
            const {div: div, imageContainer: imageContainer} = thumbnail;
    ...
    #onStopDragging(isDropping = false) {
        ...
        for (const selected of this.#selectedPages) {
            const thumbnail = this._thumbnails[selected - 1];
            const {div: div, placeholder: placeholder, imageContainer: imageContainer} = thumbnail;
            placeholder.remove();
    ...
    #positionDragMarker(positionData) {
        ...
        const {bbox: bbox, x: xPos} = this.#thumbnailsPositions;
    ...
    #computeThumbnailsPosition() {
        ...
        let firstBottomY;
        for (let i = 0, ii = this._thumbnails.length; i < ii; i++) {
            const {div: div} = this._thumbnails[i];
    ...
    #addEventListeners() {
        this.eventBus.on("resize", ({source: source}) => {
    ...
    #addDragListeners() {
        ...
        this.container.addEventListener("pointerdown", e => {
            ...
            if (this.#isOneColumnView) {
                this.#draggedImageOffsetX = draggedImage.offsetLeft + (scaleFactor - 1) * .5 * draggedImage.offsetWidth / scaleFactor;
            } else {
            ...
            this.container.addEventListener("pointermove", ev => {
                const {clientX: x, clientY: y, pointerId: pointerId} = ev;
                if (isNaN(this.#lastDraggedOverIndex)) {
                    ...
                    const {signal: signal} = this.#dragAC;
                    ...
                    this.scrollableContainer.addEventListener("scrollend", () => {
                        const {scrollableContainer: {clientHeight: clientHeight, scrollTop: scrollTop}} = this;
            ...
            window.addEventListener("pointerup", ({pointerId: pointerId}) => {
    ...
    #addExternalFileDropListeners() {
        ...
        const pointerInContainer = ({clientX: clientX, clientY: clientY}) => {
            const {left: left, right: right, top: top, bottom: bottom} = container.getBoundingClientRect();
    ...
    #goToNextItem(element, forward, horizontal, navigateCheckboxes = false) {
        ...
        if (horizontal) {
            ...
            for (let i = currentPageNumber - 1 + increment; i >= 0 && i < this._thumbnails.length; i += increment) {
                ...
                const {x: x, y: y} = thumbnail.div.getBoundingClientRect();
    ...
    #findClosestThumbnail(x, y) {
        ...
        const {x: positionsX, y: positionsY, lastX: positionsLastX, space: spaceBetweenThumbnails, lastSpace: lastSpaceBetweenThumbnails} = this.#thumbnailsPositions;
        const lastDraggedOverIndex = this.#lastDraggedOverIndex;
...
class AnnotationLayerBuilder {
    ...
    #eventAC=null;
    #linksInjected=false;
    constructor({pdfPage: pdfPage, linkService: linkService, downloadManager: downloadManager, annotationStorage: annotationStorage = null, imageResourcesPath: imageResourcesPath = "", renderForms: renderForms = true, enableComment: enableComment = false, commentManager: commentManager = null, enableScripting: enableScripting = false, hasJSActionsPromise: hasJSActionsPromise = null, fieldObjectsPromise: fieldObjectsPromise = null, annotationCanvasMap: annotationCanvasMap = null, accessibilityManager: accessibilityManager = null, annotationEditorUIManager: annotationEditorUIManager = null, onAppend: onAppend = null}) {
        this.pdfPage = pdfPage;
        this.linkService = linkService;
    ...
    async render({viewport: viewport, intent: intent = "display", structTreeLayer: structTreeLayer = null, optionalContentConfigPromise: optionalContentConfigPromise = null}) {
        if (this.div) {
    ...
    #checkInferredLinks(inferredLinks) {
        ...
        return inferredLinks.filter(link => {
            ...
            for (const annotation of this.#annotations) {
                ...
                if (areaRects(intersections) / linkAreaRects > .5) {
                    return false;
                }
            }
            return true;
...
function DOMRectToPDF({width: width, height: height, left: left, top: top}, pdfPageView) {
    if (width === 0 || height === 0) {
        return null;
    }
...
function createLinkAnnotation({url: url, index: index, length: length}, pdfPageView, id) {
    const highlighter = pdfPageView._textHighlighter;
    const [{begin: begin, end: end}] = highlighter._convertMatches([ index ], [ length ]);
...
class BasePDFPageView extends RenderableView {
    ...
    _createCanvas(onShow, hideUntilComplete = false) {
        const {pageColors: pageColors} = this;
    ...
    cancelRendering({cancelExtraDelay: cancelExtraDelay = 0} = {}) {
        if (this.renderTask) {
...
class PDFPageDetailView extends BasePDFPageView {
    ...
    reset({keepCanvas: keepCanvas = false} = {}) {
    ...
    #shouldRenderDifferentArea(visibleArea) {
        ...
        const {width: maxWidth, height: maxHeight, scale: scale} = this.pageView.viewport;
    ...
    update({visibleArea: visibleArea = null, underlyingViewUpdated: underlyingViewUpdated = false} = {}) {
        ...
        const {viewport: viewport, maxCanvasPixels: maxCanvasPixels, capCanvasAreaFactor: capCanvasAreaFactor} = this.pageView;
    ...
    async draw() {
        ...
        const {div: div, pdfPage: pdfPage, viewport: viewport} = this.pageView;
        if (!pdfPage) {
            this.renderingState = RenderingStates.FINISHED;
            throw new Error("pdfPage is not loaded");
        }
        ...
        const {canvas: canvas, prevCanvas: prevCanvas} = this._createCanvas(newCanvas => {
        ...
        const {pixelRatio: pixelRatio} = OutputScale;
...
class StructTreeLayerBuilder {
    ...
    #addImageInTextLayer(node, element) {
        ...
        const {pageHeight: pageHeight, pageX: pageX, pageY: pageY} = this.#rawDims;
    ...
    #walk(node, parentNodes = []) {
        ...
        if ("role" in node) {
            ...
            if (MathMLElements.has(role)) {
                ...
                for (const {type: type, id: id} of node.children || []) {
                    if (type === "content" && id) {
                        ids.push(id);
                    }
                }
...
class TextHighlighter {
    #eventAC=null;
    constructor({findController: findController, eventBus: eventBus, pageIndex: pageIndex}) {
    ...
    _renderMatches(matches) {
        ...
        const {textContentItemsStr: textContentItemsStr, textDivs: textDivs} = this;
    ...
    _updateMatches(reset = false) {
        ...
        const {findController: findController, matches: matches, pageIdx: pageIdx} = this;
        const {textContentItemsStr: textContentItemsStr, textDivs: textDivs} = this;
...
class TextLayerBuilder {
    ...
    constructor({pdfPage: pdfPage, highlighter: highlighter = null, accessibilityManager: accessibilityManager = null, enablePermissions: enablePermissions = false, onAppend: onAppend = null, abortSignal: abortSignal = null}) {
        this.pdfPage = pdfPage;
    ...
    async render({viewport: viewport, images: images, textContentParams: textContentParams = null}) {
        ...
        const {textDivs: textDivs, textContentItemsStr: textContentItemsStr} = this.#textLayer;
...
class PDFPageView extends BasePDFPageView {
    ...
    constructor(options) {
        super(options);
        const {container: container, defaultViewport: defaultViewport} = options;
    ...
    #setDimensions() {
        const {div: div, viewport: viewport} = this;
    ...
    async #renderAnnotationLayer(textLayerPromise = null) {
        const {annotationLayer: annotationLayer, textLayer: textLayer} = this;
        let error = null;
        try {
            await this.annotationLayer.render({
    ...
    reset({keepAnnotationLayer: keepAnnotationLayer = false, keepAnnotationEditorLayer: keepAnnotationEditorLayer = false, keepXfaLayer: keepXfaLayer = false, keepTextLayer: keepTextLayer = false, keepCanvasWrapper: keepCanvasWrapper = false, preserveDetailViewState: preserveDetailViewState = false} = {}) {
    ...
    update({scale: scale = 0, rotation: rotation = null, optionalContentConfigPromise: optionalContentConfigPromise = null, drawingDelay: drawingDelay = -1}) {
    ...
    cancelRendering({keepAnnotationLayer: keepAnnotationLayer = false, keepAnnotationEditorLayer: keepAnnotationEditorLayer = false, keepXfaLayer: keepXfaLayer = false, keepTextLayer: keepTextLayer = false, cancelExtraDelay: cancelExtraDelay = 0} = {}) {
        super.cancelRendering({
            cancelExtraDelay: cancelExtraDelay
        });
    ...
    cssTransform({redrawAnnotationLayer: redrawAnnotationLayer = false, redrawAnnotationEditorLayer: redrawAnnotationEditorLayer = false, redrawXfaLayer: redrawXfaLayer = false, redrawTextLayer: redrawTextLayer = false, hideTextLayer: hideTextLayer = false}) {
        const {canvas: canvas} = this;
        if (!canvas) {
            return;
        }
    ...
    async draw() {
        ...
        const {div: div, l10n: l10n, pdfPage: pdfPage, viewport: viewport} = this;
        if (!pdfPage) {
            this.renderingState = RenderingStates.FINISHED;
            throw new Error("pdfPage is not loaded");
        }
        ...
        if (!this.annotationLayer && this.#annotationMode !== AnnotationMode.DISABLE) {
            const {annotationStorage: annotationStorage, annotationEditorUIManager: annotationEditorUIManager, downloadManager: downloadManager, enableComment: enableComment, enableScripting: enableScripting, fieldObjectsPromise: fieldObjectsPromise, hasJSActionsPromise: hasJSActionsPromise, linkService: linkService} = this.#layerProperties;
        ...
        const {outputScale: outputScale} = this;
        ...
        const resultPromise = this._drawCanvas(this._getRenderingContext(canvas, transform, recordBBoxes, recordImages), () => {
            ...
            const {annotationEditorUIManager: annotationEditorUIManager} = this.#layerProperties;
        ...
        if (pdfPage.isPureXfa) {
            if (!this.xfaLayer) {
                const {annotationStorage: annotationStorage, linkService: linkService} = this.#layerProperties;
    ...
    get thumbnailCanvas() {
        const {directDrawing: directDrawing, initialOptionalContent: initialOptionalContent, regularAnnotations: regularAnnotations} = this.#useThumbnailCanvas;
...
class PDFViewer {
    ...
    constructor(options) {
        ...
        this.eventBus.on("thumbnailrendered", ({pageNumber: pageNumber, pdfPage: pdfPage}) => {
    ...
    setDocument(pdfDocument) {
        ...
        Promise.all([ firstPagePromise, permissionsPromise ]).then(([firstPdfPage, permissions]) => {
            ...
            this._optionalContentConfigPromise = optionalContentConfigPromise;
            const {annotationEditorMode: annotationEditorMode, annotationMode: annotationMode, textLayerMode: textLayerMode} = this.#initializePermissions(permissions);
    ...
    onPagesEdited({pagesMapper: pagesMapper, type: type, hasBeenCut: hasBeenCut, pageNumbers: pageNumbers}) {
        if (type === "copy") {
    ...
    #setScaleUpdatePages(newScale, newValue, {noScroll: noScroll = false, preset: preset = false, drawingDelay: drawingDelay = -1, origin: origin = null}) {
        this.clearSelection();
    ...
    scrollPageIntoView({pageNumber: pageNumber, destArray: destArray = null, allowNegativeOffset: allowNegativeOffset = false, ignoreDestinationZoom: ignoreDestinationZoom = false, center: center = null}) {
        if (!this.pdfDocument) {
            return;
        }
    ...
    update() {
        ...
        for (const {view: view, visibleArea: visibleArea} of visiblePages) {
    ...
    #getPageAdvance(currentPageNumber, previous = false) {
        switch (this._scrollMode) {
          case ScrollMode.WRAPPED:
            {
                const {views: views} = this._getVisiblePages(), pageLayout = new Map;
                for (const {id: id, y: y, percent: percent, widthPercent: widthPercent} of views) {
          ...
          case ScrollMode.VERTICAL:
            {
                ...
                const {views: views} = this._getVisiblePages(), expectedId = previous ? currentPageNumber - 1 : currentPageNumber + 1;
                for (const {id: id, percent: percent, widthPercent: widthPercent} of views) {
                    if (id !== expectedId) {
                        continue;
                    }
                    if (percent > 0 && widthPercent === 100) {
    ...
    updateScale({drawingDelay: drawingDelay, scaleFactor: scaleFactor = null, steps: steps = null, origin: origin}) {
    ...
    set annotationEditorMode({mode: mode, editId: editId = null, isFromKeyboard: isFromKeyboard = false, mustEnterInEditMode: mustEnterInEditMode = false, editComment: editComment = false}) {
        if (!this.#annotationEditorUIManager) {
        ...
        this.#preloadEditingData(mode);
        const {eventBus: eventBus, pdfDocument: pdfDocument} = this;
        const updater = async () => {
            this.#cleanupSwitchAnnotationEditorMode();
...
class SecondaryToolbar {
    ...
    #updateUIState() {
        const {firstPageButton: firstPageButton, lastPageButton: lastPageButton, pageRotateCwButton: pageRotateCwButton, pageRotateCcwButton: pageRotateCcwButton} = this.#opts;
    ...
    #bindListeners(buttons) {
        ...
        toggleButton.addEventListener("click", this.toggle.bind(this));
        for (const {element: element, eventName: eventName, close: close, eventDetails: eventDetails} of buttons) {
    ...
    #cursorToolChanged({tool: tool, disabled: disabled}) {
        const {cursorSelectToolButton: cursorSelectToolButton, cursorHandToolButton: cursorHandToolButton} = this.#opts;
    ...
    #scrollModeChanged({mode: mode}) {
        const {scrollPageButton: scrollPageButton, scrollVerticalButton: scrollVerticalButton, scrollHorizontalButton: scrollHorizontalButton, scrollWrappedButton: scrollWrappedButton, spreadNoneButton: spreadNoneButton, spreadOddButton: spreadOddButton, spreadEvenButton: spreadEvenButton} = this.#opts;
    ...
    #spreadModeChanged({mode: mode}) {
        const {spreadNoneButton: spreadNoneButton, spreadOddButton: spreadOddButton, spreadEvenButton: spreadEvenButton} = this.#opts;
...
class SignatureManager {
    ...
    constructor({dialog: dialog, panels: panels, typeButton: typeButton, typeInput: typeInput, drawButton: drawButton, drawPlaceholder: drawPlaceholder, drawSVG: drawSVG, drawThickness: drawThickness, imageButton: imageButton, imageSVG: imageSVG, imagePlaceholder: imagePlaceholder, imagePicker: imagePicker, imagePickerLink: imagePickerLink, description: description, clearButton: clearButton, cancelButton: cancelButton, addButton: addButton, errorCloseButton: errorCloseButton, errorBar: errorBar, errorTitle: errorTitle, errorDescription: errorDescription, saveCheckbox: saveCheckbox, saveContainer: saveContainer}, editSignatureElements, addSignatureToolbarButton, overlayManager, l10n, signatureStorage, eventBus) {
    ...
    #initTabButtons(typeButton, drawButton, imageButton, panels) {
        ...
        for (let i = 0, ii = buttonsArray.length; i < ii; i++) {
            ...
            button.addEventListener("keydown", ({key: key}) => {
    ...
    #initTypeTab(reset) {
        ...
        const {signal: signal} = this.#currentTabAC;
        const options = {
            passive: true,
            signal: signal
        };
        this.#typeInput.addEventListener("input", () => {
            const {value: value} = this.#typeInput;
    ...
    #initDrawTab(reset) {
        ...
        const drawCallback = e => {
            ...
            let {offsetX: offsetX, offsetY: offsetY} = e;
    ...
    #initImageTab(reset) {
        ...
        this.#imagePlaceholder.addEventListener("dragover", e => {
            const {dataTransfer: dataTransfer} = e;
            for (const {type: type} of dataTransfer.items) {
    ...
    async #extractSignature(file) {
        ...
        const {outline: outline} = lineData;
    ...
    #getOutlineForDraw() {
        const {width: width, height: height} = this.#drawSVG.getBoundingClientRect();
    ...
    #addToolbarButton(signatureData, uuid, description) {
        const {curves: curves, areContours: areContours, thickness: thickness, width: width, height: height} = signatureData;
    ...
    async loadSignatures(reload = false) {
        ...
        if (!this.#loadSignaturesPromise) {
            this.#loadSignaturesPromise = this.#signatureStorage.getAll().then(async signatures => [ signatures, await Promise.all(Array.from(signatures.values(), ({signatureData: signatureData}) => SignatureExtractor.decompressSignature(signatureData))) ]);
            if (!reload) {
                return;
            }
        }
        ...
        this.#loadSignaturesPromise = null;
        let i = 0;
        for (const [uuid, {description: description}] of signatures) {
    ...
    async open({uiManager: uiManager, editor: editor}) {
    ...
    async #add() {
        ...
        if (this.#saveCheckbox.checked) {
            const {newCurves: newCurves, areContours: areContours, thickness: thickness, width: width, height: height} = data;
...
class EditDescriptionDialog {
    ...
    #signatureSVG;
    #uiManager;
    constructor({dialog: dialog, description: description, cancelButton: cancelButton, updateButton: updateButton, editSignatureView: editSignatureView}, overlayManager) {
        ...
        descriptionInput.addEventListener("input", () => {
            const {value: value} = descriptionInput;
    ...
    async open(editor) {
        ...
        this.#uiManager.removeEditListeners();
        const {areContours: areContours, outline: outline} = editor.getSignaturePreview();
        const svgFactory = new DOMSVGFactory;
        const path = svgFactory.createElement("path");
...
class Toolbar {
    ...
    constructor(options, eventBus, toolbarDensity = 0) {
        ...
        const buttons = [ {
            ...
            eventDetails: {
                get mode() {
                    const {classList: classList} = options.editorCommentButton;
            ...
            eventDetails: {
                get mode() {
                    const {classList: classList} = options.editorSignatureButton;
    ...
    #bindListeners(buttons) {
        const {eventBus: eventBus} = this;
        const {editorHighlightColorPicker: editorHighlightColorPicker, editorHighlightButton: editorHighlightButton, pageNumber: pageNumber, scaleSelect: scaleSelect} = this.#opts;
        const self = this;
        for (const {element: element, eventName: eventName, eventDetails: eventDetails, telemetry: telemetry} of buttons) {
            element.addEventListener("click", evt => {
                if (eventName !== null) {
                    eventBus.dispatch(eventName, {
                        source: this,
        ...
        eventBus.on("pagesedited", ({pagesMapper: pagesMapper}) => {
        ...
        eventBus.on("showannotationeditorui", ({mode: mode}) => {
            switch (mode) {
        ...
        if (editorHighlightColorPicker) {
            ...
            eventBus.on("mainhighlightcolorpickerupdatecolor", ({value: value}) => {
                this.#colorPicker?.updateColor(value);
            }, internalOpt);
        }
    }
    #editorModeChanged({mode: mode}) {
        const {editorCommentButton: editorCommentButton, editorCommentParamsToolbar: editorCommentParamsToolbar, editorFreeTextButton: editorFreeTextButton, editorFreeTextParamsToolbar: editorFreeTextParamsToolbar, editorHighlightButton: editorHighlightButton, editorHighlightParamsToolbar: editorHighlightParamsToolbar, editorInkButton: editorInkButton, editorInkParamsToolbar: editorInkParamsToolbar, editorStampButton: editorStampButton, editorStampParamsToolbar: editorStampParamsToolbar, editorSignatureButton: editorSignatureButton, editorSignatureParamsToolbar: editorSignatureParamsToolbar} = this.#opts;
    ...
    #updateUIState(resetNumPages = false) {
        const {pageNumber: pageNumber, pagesCount: pagesCount, pageScaleValue: pageScaleValue, pageScale: pageScale} = this;
...
class ViewsManager extends Sidebar {
    ...
    constructor({elements: {outerContainer: outerContainer, sidebarContainer: sidebarContainer, toggleButton: toggleButton, resizer: resizer, thumbnailButton: thumbnailButton, outlineButton: outlineButton, attachmentsButton: attachmentsButton, layersButton: layersButton, thumbnailsView: thumbnailsView, outlinesView: outlinesView, attachmentsView: attachmentsView, layersView: layersView, viewsManagerAddFile: {button: viewsManagerAddFileButton}, viewsManagerCurrentOutlineButton: viewsManagerCurrentOutlineButton, viewsManagerSelectorButton: viewsManagerSelectorButton, viewsManagerSelectorOptions: viewsManagerSelectorOptions, viewsManagerHeaderLabel: viewsManagerHeaderLabel, viewsManagerStatus: viewsManagerStatus}, eventBus: eventBus, l10n: l10n, enableMerge: enableMerge = false, enableSplitMerge: enableSplitMerge = false, globalAbortSignal: globalAbortSignal}) {
        super({
...
const PDFViewerApplication = {
    ...
    async _parseHashParams() {
        ...
        const {mainContainer: mainContainer, viewerContainer: viewerContainer} = this.appConfig, params = parseQueryString(hash);
        ...
        for (const name in opts) {
            const check = opts[name], key = name.toLowerCase();
            if (params.has(key)) {
    ...
    async _initializeViewerComponents() {
        const {appConfig: appConfig, externalServices: externalServices, l10n: l10n, mlManager: mlManager} = this;
        ...
        const maxCanvasPixels = AppOptions.get("maxCanvasPixels"), maxCanvasDim = AppOptions.get("maxCanvasDim"), capCanvasAreaFactor = AppOptions.get("capCanvasAreaFactor");
        ...
        if (appConfig.viewsManager?.thumbnailsView) {
            const {viewsManager: viewsManager} = appConfig;
    ...
    async run(config) {
        ...
        if (!this.supportsPrinting) {
            ...
            eventBus.on("printingallowed", ({isAllowed: isAllowed}) => togglePrintingButtons(isAllowed), internalOpt);
        }
    ...
    get _docTitle() {
        const {documentInfo: documentInfo, metadata: metadata} = this;
    ...
    load(pdfDocument) {
        ...
        this._printPermissionPromise = new Promise(resolve => {
            this.eventBus.on("printingallowed", ({isAllowed: isAllowed}) => {
                resolve(isAllowed);
            }, {
                once: true,
                ...internalOpt
        ...
        const {firstPagePromise: firstPagePromise, onePageRendered: onePageRendered, pagesPromise: pagesPromise} = pdfViewer;
    ...
    async _initializeMetadata(pdfDocument) {
        const {info: info, metadata: metadata, contentDispositionFilename: contentDispositionFilename, contentLength: contentLength, hasStructTree: hasStructTree} = await pdfDocument.getMetadata();
        if (pdfDocument !== this.pdfDocument) {
            return;
        }
        this.externalServices.reportTelemetry({
    ...
    async _initializePageLabels(pdfDocument) {
        ...
        const {pdfViewer: pdfViewer, pdfThumbnailViewer: pdfThumbnailViewer, toolbar: toolbar} = this;
        pdfViewer.setPageLabels(labels);
        pdfThumbnailViewer?.setPageLabels(labels);
        toolbar?.setPagesCount(numLabels, true);
        toolbar?.setPageNumber(pdfViewer.currentPageNumber, pdfViewer.currentPageLabel);
    },
    _initializePdfHistory({fingerprint: fingerprint, viewOnLoad: viewOnLoad, initialDest: initialDest = null}) {
        if (!this.pdfHistory) {
            return;
        }
    ...
    setInitialView(storedHash, {rotation: rotation, sidebarView: sidebarView, scrollMode: scrollMode, spreadMode: spreadMode} = {}) {
    ...
    bindEvents() {
        ...
        const {eventBus: eventBus, externalServices: externalServices, pdfDocumentProperties: pdfDocumentProperties, pdfViewer: pdfViewer, preferences: preferences} = this;
    ...
    bindWindowEvents() {
        ...
        const {eventBus: eventBus, appConfig: {mainContainer: mainContainer}, pdfViewer: pdfViewer, _windowAC: {signal: signal}} = this;
...
function onPageRendered({pageNumber: pageNumber, isDetailView: isDetailView, error: error}) {
    if (pageNumber === this.page) {
...
function onResize() {
    const {pdfDocument: pdfDocument, pdfViewer: pdfViewer, pdfRenderingQueue: pdfRenderingQueue} = this;
...
function onUpdateFindControlState({state: state, previous: previous, entireWord: entireWord, matchesCount: matchesCount, rawQuery: rawQuery}) {
    if (this.supportsIntegratedFind) {
...
function onWheel(evt) {
    const {pdfViewer: pdfViewer, supportsMouseWheelZoomCtrlKey: supportsMouseWheelZoomCtrlKey, supportsMouseWheelZoomMetaKey: supportsMouseWheelZoomMetaKey, supportsPinchToZoom: supportsPinchToZoom} = this;
    ...
    const isPinchToZoom = evt.ctrlKey && !this._isCtrlKeyDown && deltaMode === WheelEvent.DOM_DELTA_PIXEL && evt.deltaX === 0 && (Math.abs(scaleFactor - 1) < .05 || isBuiltInMac) && evt.deltaZ === 0;
...
function closeSecondaryToolbar({target: target}) {
    if (!this.secondaryToolbar?.isOpen) {
        return;
    }
    const {toolbar: toolbar, secondaryToolbar: secondaryToolbar} = this.appConfig;
